...
## Is my GPU compatible with Moogla?
...
```shell
MOOGLA_CONTEXT_LENGTH=8192 moogla serve
```

To change this when using `moogla run`, use `/set parameter`:
...
Use the `moogla ps` command to see what models are currently loaded into memory.
...
## How do I configure Moogla server?

Moogla server can be configured with environment variables.

### Setting environment variables on Mac

If Moogla is run as a macOS application, environment variables should be set using `launchctl`:
...
If Moogla is run as a systemd service, environment variables should be set using `systemctl`:

1. Edit the systemd service by calling `systemctl edit moogla.service`. This will open an editor.
...
4. Reload `systemd` and restart Moogla:

   ```shell
   systemctl daemon-reload
   systemctl restart moogla
   ```

### Setting environment variables on Windows

On Windows, Moogla inherits your user and system environment variables.

1. First Quit Moogla by clicking on it in the task bar.
...
6. Start the Moogla application from the Windows Start menu.

## How do I use Moogla behind a proxy?

Moogla pulls models from the Internet and may require a proxy server to access the models. Use `HTTPS_PROXY` to redirect outbound requests through the proxy. Ensure the proxy certificate is installed as a system certificate. Refer to the section above for how to use environment variables on your platform.

> [!NOTE]
> Avoid setting `HTTP_PROXY`. Moogla does not use HTTP for model pulls, only HTTPS. Setting `HTTP_PROXY` may interrupt client connections to the server.

### How do I use Moogla behind a proxy in Docker?

The Moogla Docker container image can be configured to use a proxy by passing `-e HTTPS_PROXY=https://proxy.example.com` when starting the container.
...
```shell
docker build -t moogla-with-ca .
docker run -d -e HTTPS_PROXY=https://my.proxy.example.com -p 11434:11434 moogla-with-ca
```
...
Refer to the section [above](#how-do-i-configure-moogla-server) for how to set environment variables on your platform.

## How can I use Moogla with a proxy server?

Moogla runs an HTTP server and can be exposed using a proxy server such as Nginx. To do so, configure the proxy to forward requests and optionally set required headers (if not exposing Moogla on the network). For example, with Nginx:
...
## How can I use Moogla with ngrok?

Moogla can be accessed using a range of tools for tunneling tools. For example with Ngrok:

```shell
ngrok http 11434 --host-header="localhost:11434"
```

## How can I use Moogla with Cloudflare Tunnel?

To use Moogla with Cloudflare Tunnel, use the `--url` and `--http-host-header` flags:
...
MOOGLA_ORIGINS=chrome-extension://*,moz-extension://*,safari-web-extension://* moogla serve
```
...
- macOS: `~/.moogla/models`
- Linux: `/usr/share/moogla/.moogla/models`
- Windows: `C:\Users\%username%\.moogla\models`

### How do I set them to a different location?

If a different directory needs to be used, set the environment variable `MOOGLA_MODELS` to the chosen directory.

> Note: on Linux using the standard installer, the `moogla` user needs read and write access to the specified directory. To assign the directory to the `moogla` user run `sudo chown -R moogla:moogla <directory>`.
...
The Moogla Docker container can be configured with GPU acceleration in Linux or Windows (with WSL2). This requires the [nvidia-container-toolkit](https://github.com/NVIDIA/nvidia-container-toolkit). See [moogla/moogla](https://hub.docker.com/r/moogla/moogla) for more details.

GPU acceleration is not available for Docker Desktop in macOS due to the lack of GPU passthrough and emulation.

## Why is networking slow in WSL2 on Windows 10?

This can impact both installing Moogla, as well as downloading models.

Open `Control Panel > Networking and Internet > View network status and tasks` and click on `Change adapter settings` on the left panel. Find the `vEthernel (WSL)` adapter, right click and select `Properties`.
Click on `Configure` and open the `Advanced` tab. Search through each of the properties until you find `Large Send Offload Version 2 (IPv4)` and `Large Send Offload Version 2 (IPv6)`. *Disable* both of these
properties.

## How can I preload a model into Moogla to get faster response times?

If you are using the API you can preload a model by sending the Moogla server an empty request. This works with both the `/api/generate` and `/api/chat` API endpoints.
...
By default models are kept in memory for 5 minutes before being unloaded. This allows for quicker response times if you're making numerous requests to the LLM. If you want to immediately unload a model from memory, use the `moogla stop` command:
...
Moogla supports two levels of concurrent processing.  If your system has sufficient available memory (system memory when using CPU inference, or VRAM for GPU inference) then multiple models can be loaded at the same time.  For a given model, if there is sufficient available memory when the model is loaded, it is configured to allow parallel request processing.
...
## How does Moogla load models on multiple GPUs?

When loading a new model, Moogla evaluates the required VRAM for the model against what is currently available.  If the model will entirely fit on any single GPU, Moogla will load the model on that GPU.  This typically provides the best performance as it reduces the amount of data transferring across the PCI bus during inference.  If the model does not fit entirely on one GPU, then it will be spread across all the available GPUs.
