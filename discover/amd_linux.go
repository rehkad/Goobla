...
func AMDGetGPUInfo() ([]RocmGPUInfo, error) {
	...
	if err != nil {
		...
		slog.Warn("goobla recommends running the https://www.amd.com/en/support/linux-drivers", "error", err)
	}
	...
	for _, match := range matches {
		...
		if gfxOverride == "" {
			...
			if !slices.Contains[[]string, string](supported, gfx) {
				...
				slog.Warn("See https://github.com/goobla/goobla/blob/main/docs/gpu.md#overrides for HSA_OVERRIDE_GFX_VERSION usage")
				continue
			} else {
...
func AMDValidateLibDir() (string, error) {
	...
	// Well known goobla installer path
	installedRocmDir := "/usr/share/goobla/lib/rocm"
	if rocmLibUsable(installedRocmDir) {
		return installedRocmDir, nil
	}

	// If we still haven't found a usable rocm, the user will have to install it on their own
	slog.Warn("amdgpu detected, but no compatible rocm library found.  Either install rocm v6, or follow manual install instructions at https://github.com/goobla/goobla/blob/main/docs/linux.md#manual-install")
...
func verifyKFDDriverAccess() error {
	...
	if err != nil {
		if errors.Is(err, fs.ErrPermission) {
			return fmt.Errorf("permissions not set up properly.  Either run goobla as root, or add you user account to the render group. %w", err)
