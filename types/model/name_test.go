...
func TestParseNameParts(t *testing.T) {
	cases := []struct {
		...
		{
			in: "registry.goobla.ai/library/dolphin-mistral:7b-v2.6-dpo-laser-q6_K",
			want: Name{
				Host:      "registry.goobla.ai",
				Namespace: "library",
				Model:     "dolphin-mistral",
				Tag:       "7b-v2.6-dpo-laser-q6_K",
			},
			wantFilepath: filepath.Join("registry.goobla.ai", "library", "dolphin-mistral", "7b-v2.6-dpo-laser-q6_K"),
		},
		{
		...
		},
		{
			...
			wantFilepath: filepath.Join("registry.goobla.ai", "namespace", "model", "latest"),
		},
		{
			in: "model",
			want: Name{
			...
			wantFilepath: filepath.Join("registry.goobla.ai", "library", "model", "latest"),
		},
		{
			in: "h/nn/mm:t",
			want: Name{
...
func TestNameparseNameDefault(t *testing.T) {
	...
	n := ParseName(name)
	got := n.String()
	want := "registry.goobla.ai/library/xx:latest"
	if got != want {
...
func TestDisplayShortest(t *testing.T) {
	cases := map[string]string{
		"registry.goobla.ai/library/model:latest": "model:latest",
		"registry.goobla.ai/library/model:tag":    "model:tag",
		"registry.goobla.ai/namespace/model:tag":  "namespace/model:tag",
